...
class MetadataManager:
    ...
    def get_next_partition(self, consumer_id):
        return int(self.rc.lmove(first_list=f"consumer:{consumer_id}:partition_queue", second_list=f"consumer:{consumer_id}:partition_queue"))
    ...
    def get_all_topics(self):
        topic_keys = self.rc.keys(pattern="topic:*")
        return [topic_key.split(":")[1] for topic_key in topic_keys]
    ...
    def get_consumer_offset(self, consumer_id, partition_index):
        return int(self.rc.hget(name=f"consumer:{consumer_id}:offset", key=partition_index))
